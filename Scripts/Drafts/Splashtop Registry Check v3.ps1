...
$secureCred = New-Object System.Management.Automation.PSCredential('SMTPName', $secureString);
...
{
0 {Send-MailMessage -To 'name@company.com' -Subject 'Splashtop registry setting' -Body "WARNING: The Splashtop Streamer settings for $localUsers on computer $env:COMPUTERNAME are not configured to automatically lock the computer at the end of a session. Splashtop settings are now updating to correct this." -Cc 'name@company.com' -From 'name@company.com' -Credential $secureCred -Port 587}
